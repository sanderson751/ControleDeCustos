...
- RNF-04: O estado visual do menu deve ser previsivel e consistente apos recarregamento com persistencia no Firestore.
...
- RNF-07: A interface deve manter legibilidade em tema `light` e `dark` com contraste minimo AA.
- RNF-08: Textos criticos e controles interativos nao devem usar cor fixa em contextos tematicos.
- RNF-09: Operacoes com resultado de erro/sucesso/warning devem exibir snackbar no canto superior direito.
- RNF-10: Snackbar deve permanecer visivel por 3 segundos e permitir fechar com icone `X`.
- RNF-11: Cores de snackbar devem seguir status: vermelho (erro), verde (sucesso), amarelo (warning).
- RNF-12: A preferencia de layout (ex.: sidebarCollapsed) deve persistir no Firestore como fonte final da verdade.
- RNF-13: Falhas de persistencia de preferencia devem ser tratadas com fallback seguro e feedback ao usuario.
