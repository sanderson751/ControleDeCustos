...
export async function getUserRole(userId: string): Promise<UserRole> {
  try {
    ...
    if (!userDoc.exists()) {
      console.warn(`Usuário ${userId} não encontrado no Firestore`);
      return 'standard'; // fallback padrão
    }

    const role = userDoc.data().role as UserRole | undefined;

    // validar se role é valor válido
    if (role && ['admin', 'standard', 'guest'].includes(role)) {
      return role;
    }

    console.warn(`Role inválida para usuário ${userId}:`, role, 'usando fallback standard');
    return 'standard';
  } catch (error) {
    console.error('Erro ao carregar role do usuário:', error);
    return 'standard'; // fallback em caso de erro
  }
}
